...
def main(strat1,strat2):
    ...
    for i in ttl_ttl_points:
        ...
        y = []
        for elt in i:
            x.append(np.mean(elt[:4]))
            y.append(np.mean(elt[4:]))
        ttl_x.append(x)
        ttl_y.append(y)
    ...
    plt.title(f"Comparaison 4 {namestrat2} vs 4 {namestrat1}")
    plt.plot(a, color='red',label=f"moyenne {namestrat2}")
    plt.plot(b, color='blue',label=f"moyenne {namestrat1}")
    plt.xlabel("nb iter")
    plt.ylabel("nb points")
    plt.legend()
    # plt.show()
    plt.savefig(f"graphs/{nb_iter}it_{nb_jour}jour_4{namestrat2}_vs_4{namestrat1}.png")
